...
describe('POST /api/generate-pptx', () => {
  it('generates preview data without writing a file to disk', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
    // No downloadUrl — generate-pptx is preview-only, file generation is via patch apply
...
describe('GET /api/download/:filename', () => {
  ...
  it('serves a file generated by patch apply', async () => {
    // File generation now only happens via patch apply — verify the download
    // route serves the patch output correctly
    const templatePath = await uploadFixture();

    // Create chain and apply a patch to produce a real output file
    ...
    const applyRes = await request(app)
      ...
      .send({ tags: [], jsonData: {}, repeatableSlides: [], roundName: 'Test' });
    expect(applyRes.status).toBe(200);

    const filename = path.basename(applyRes.body.downloadUrl);
    const res = await request(app)
      .get(`/api/download/${filename}`)
      .buffer(true)
      .parse((res, cb) => {
