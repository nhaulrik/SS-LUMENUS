...
import {
  ...
  forkExport,
  updateSlideTitle,
} from '../lib/project/export-manager.js';
...
setInterval(() => {
  const now = Date.now();
  for (const [id, entry] of pendingTemplates) {
    if (entry.expiresAt && entry.expiresAt < now) pendingTemplates.delete(id);
  }
...
router.patch('/html-flow/update-selections', (req, res) => {
  try {
    ...
    session.expiresAt = Date.now() + PENDING_TEMPLATE_TTL_MS;
